...
(function() {
  ...
  window.Resumable = Resumable = (function() {
    ...
    function Resumable(opt) {
      ...
      this.defaults = {
        ...
        maxFilesErrorCallback: function(files, errorCount) {
          ...
          return dms.notification.info('최대 업로드 파일 갯수는' + maxFiles + ' 개 입니다');
        },
        minFileSize: void 0,
        minFileSizeErrorCallback: function(file, errorCount) {
          ...
          return dms.notification.info('파일 사이즈가 너무 작습니다. 업로드 파일은' + that.formatSize(that.getOpt('minFileSize')) + '이상 이어야 합니다.');
        },
        ...
        maxFileSizeErrorCallback: function(file, errorCount) {
          ...
          return dms.notification.info('파일 사이즈가 너무 큽니다. 업로드 파일은' + that.formatSize(that.getOpt('maxFileSize')) + '이하 이어야 합니다.');
        },
		fileType: ["7z","alz","doc","docx","gif","jpg","jpeg","pdf","png","ppt","pptx","text","txt","xls","xlsx","zip","avi","wmv","mpeg","mpg","mkv","mp4","tp","ts","mov","swf","flv","mp3","rar","png","sql","m4a"],
		fileTypeErrorCallback: function(file, errorCount) {
			//alert(file.fileName||file.name +' has type not allowed, please upload files of type ' + $.getOpt('fileType') + '.');
		    //선택한 파일은 업로드 할 수 없습니다. 업로드 가능한 파일 유형[xlx, pptx]
			dms.notification.info('선택한 파일은 업로드 할 수 없습니다. 업로드 가능한 파일 유형 [' + that.getOpt('fileType') + '] ');
		}
      };
      if (this.opt == null) {
        this.opt = {};
    ...
    Resumable.prototype.onDrop = function(event) {
      ...
      return that.appendFilesFromFileList(event.dataTransfer.files, event);
    };
...
//      _ref = this.getOpt(['maxFiles', 'minFileSize', 'maxFileSize', 'maxFilesErrorCallback', 'minFileSizeErrorCallback', 'maxFileSizeErrorCallback']),
      ...
      if(dms.string.isEmpty(this.loadedfiles))this.loadedfiles = 0;
      if ((maxFiles != null) && maxFiles < (fileList.length + this.loadedfiles)) {
    ...
    Resumable.prototype.assignBrowse = function(domNodes, isDirectory, customChangeHandler) {
    ...
    Resumable.prototype.upload = function() {
      ...
      this.opt.query = Object.assign(this.defaults.query, this.opt.query);
      _results = [];
