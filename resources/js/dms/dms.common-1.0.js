...
// 입력폼 숫자,영문,툭수문자만 입력
$(document).on('keyup','.onlyAlphaNumSpec', function() {
	$(this).val( $(this).val().replace(/[ㄱ-ㅎㅏ-ㅣ가-힣]/gi,"") );
});
$(document).on('focusout','.onlyAlphaNumSpec', function() {
	$(this).val( $(this).val().replace(/[ㄱ-ㅎㅏ-ㅣ가-힣]/gi,"") );
});

// 입력폼 숫자, '-' 만 입력
$(document).on('keyup','.onlyNumMinus', function() {
	$(this).val( $(this).val().replace(/[^0-9-]/gi,"") );
});
$(document).on('focusout','.onlyNumMinus', function() {
	$(this).val( $(this).val().replace(/[^0-9-]/gi,"") );
});
...
// 입력폼 한글,영문, 숫자만 입력
$(document).on('keyup','.onlyKoeanAlphaNum', function() {
    $(this).val( $(this).val().replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/ ]/gim,"") );
});
$(document).on('focusout','.onlyKoeanAlphaNum', function() {
	$(this).val( $(this).val().replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/ ]/gim,"") );
});

// 입력폼 한글, 영문, 숫자, 스페이스만 입력
$(document).on('keyup','.onlyKorAlphaNumSpace', function() {
    $(this).val( $(this).val().replace(/[^ ㄱ-ㅎ가-힣a-zA-Z0-9]/gi,"") );
});
$(document).on('focusout','.onlyKorAlphaNumSpace', function() {
	$(this).val( $(this).val().replace(/[^ ㄱ-ㅎ가-힣a-zA-Z0-9]/gi,"") );
});
...
(function(window, document, $) {
    ...
    dms.window = {
        ...
        popup:function(options){
            this.settings = {
                ...
                ,open:function(e){
                    ...
                    if(e.sender.options.iframe){
						var maxWidth;
                        var iframeLoader = $("<div id='iframe-loader'/>");
			...
			if(!this.settings.modal){
	            if (window.parent) {
	                if (dms.browser.isTablet()) { // tablet 에서 팝업사이즈가 큰경우 tab-iframe 사이즈를 넘어가는 문제때문에 절대값으로 수정
	                    _popupWindowContainer.closest(".k-window").css({
	                        top: 40, left: 20, maxWidth: 960
	                    });
	                } else {
	                    //khs 문제시 삭제
        ...
        popupPostal:function(options) {
			...
			try{
				...
				dms.notification.error("주소 API 호출이 되지않았습니다. 새로고침후 진행해주세요.");
				return;
			}
        ...
        confirm:function(options){
			...
			// 엔터키 입력시 자동 확인
			if( options.enterConfirm ){
				kendo.ui.Window.fn._keydown = function(originalFn) {
		    	    var KEY_ENTER = 13;
		    	    return function(e) {
		    	        if (e.which === KEY_ENTER) {
							kendoWindow.data("kendoWindow").close();
                			settings.callback(options.enterConfirm);
		    	        }
		    	    };
		    	}(kendo.ui.Window.fn._keydown);
			...
			switch(type){
				case 'regNo':
                    if(length < 13){
                        ...
                        regExp = /(\d{6})(\d|\*{7})/
                        rtnVal = strVal.replace(regExp, '$1-$2')
                    }
				    break;
                case 'corpRegNo' :
                    regExp = /(\d{4})(\d{2})(\d{6})/
    ...
    dms.ws = {
        connect:function(){
          let socket = new SockJS(_contextPath +'/wdms-wsc');//웹소켓 연결
          return socket;
        }
    };
    dms.loading = {
        addProgressBarPopup:function(target,backgroundColor){

            if(target === "undefined" || target == null){
                target = $("body")[0];
            }
            let classLoadingMask =  $(target).find("progressExcelWindow");
            if( classLoadingMask.length == 0 )
            {
                $("<div/>").appendTo(target)
                      .append("<div id=\"progressExcelWindow\" class=\"resumable-progress window-content scroll-hidden2 bg\" style=\"display:none;\">"
                                            ...
                                            +"<div class=\"item-body-box\">"
                                                +"<div class=\"file-send-wrap\">"
                                                    +"<div class=\"file-send-header\">"
                                                        +"<div class=\"file-graph-area\">"
                                                            +"<div class=\"graph-header\">"
                                                                +"<div class=\"file-status-text\">진행 중...</div>"
                                                                +"<div class=\"send-percent\">"
                                                                    +"<span class=\"percent-num progress-txt\">0%</span>"
                                                                +"</div>"
                                                            +"</div>"
                                                            +"<div class=\"graph-body\">"
                                                                +"<div class=\"file-graph\">"
                                                                    +"<span class=\"graph-bar progress-bar\"></span>"
                                                                +"</div>"
                                                            +"</div>"
                                                        +"</div>"
                                                    +"</div>"
        ...
        show:function(target, backgroundColor){
            ...
            $("<div/>").appendTo(target)
                ...
                .append("<div class=\"k-loading-image\" style=\"url('"+_contextPath+"/resources/img/loading-image.gif') no-repeat scroll 50% 50%\"></div>")
                .append("<div class=\"k-loading-color\"></div>");
        },
        excelExportByPoiPopupShow:function(){
              $('.file-status-text').html('데이터 조회 중.');
              $('.progress-txt').html('10%');
              $('.progress-bar').css({width:'10%'});
              $("#progressExcelWindow").kendoWindow({
              		width:"600px"
              		,height:"50px"
              		,title:"엑셀 Export"
              		,animation:false
        ...
        txtHide:function(target) {
            $("#progressExcelWindow").data("kendoWindow").close();
        },
    ...
    dms.ajax = {
        ...
        subscribeAlarm:function(params, callBackFunc){
              function connect() {
                   let socket = dms.ws.connect();
                   let stompClient = Stomp.over(socket);

                   stompClient.debug = () => {}; // 디버그 콘솔 삭제
                   stompClient.connect({}, function () {
                       // 연결이 성공했을 때 실행할 코드
                       let substr = "/wdms-subscribe/" + params['corpCd'] + "/" + params['brchCd'] + "/" + params['usrId'];
                       stompClient.subscribe(substr, function (subData) {
                           // 서버로부터 메시지를 받았을 때 실행할 코드
                           const obj = JSON.parse(subData.body);
                           // DOM 조작 및 콜백 함수 실행
                           $(".gnb-bottom a.gnb-user").addClass("active");
                           $(".gnb-bottom a.gnb-user").click(function(){
                               $(".gnb-bottom a.gnb-user").removeClass("active");
                           });
                           callBackFunc();
                       });
                       ...
                       // 연결 실패 혹은 연결이 끊어졌을 때 실행할 코드
                       console.log('Connection lost: STOMP DISCONNECT');
                       // 재연결을 시도하기 전에 일정 시간을 기다림
...
//            var url = _contextPath + "/cmm/sci/excelDownloadByPoi.do";
        ...
        excelExportByPoiPostTxt:function(params) {
            //progressBar popup 생성
            dms.loading.addProgressBarPopup();
            let socket = dms.ws.connect();
            let stompClient = Stomp.over(socket);
            let substr = '';
            stompClient.debug = () => {};
            stompClient.connect({}, function () {
                setTimeout(function(){
                    let _transportUrl = socket._transport.url
                    const sessionId = _transportUrl.split('/');
                    substr = "/wdms-subscribe/progressbar/"+params['beanName']+"/"+sessionId[sessionId.length-2];
                    stompClient.subscribe(substr, function (subData) {
                        const obj = JSON.parse(subData.body);
                        $('.progress-txt').html(obj.percentage+'%');

                        if(obj.statusMsg != null){
                            $('.file-status-text').html(obj.statusMsg);
                        }

                        $('.progress-bar').css({width:obj.percentage+'%'});
                        //100%가 되면 팝업을 내리고 connect 종료
          ...
          setTimeout(function(){
            ...
            var url = _contextPath + "/cmm/sci/excelDownloadByPoi.do";
            dms.loading.excelExportByPoiPopupShow();
            $.fileDownload(url, {
               httpMethod: "post",
        ...
        excelExportByPoiTxt:function(params) {
            //progressBar popup 생성
            dms.loading.addProgressBarPopup();
            let socket = dms.ws.connect();
            let stompClient = Stomp.over(socket);
        ...
        excelEncryptExportByPoiTxt:function(params) {
            //progressBar popup 생성
            dms.loading.addProgressBarPopup();
            let socket = dms.ws.connect();
            let stompClient = Stomp.over(socket);
          ...
          setTimeout(function(){
            ...
            var url = _contextPath + "/cmm/sci/excelEncryptDownloadByPoi.do?" + queryString;
    ...
    dms.fileManager = {
        ...
        ,download: function (fileDocNo, fileNo, downloadDocType = "cmm", ubiYn) {
            if (!dms.fileManager.exist(fileDocNo, fileNo, downloadDocType, ubiYn)) {
                dms.notification.error(dms.settings.defaultMessage.fileNotFound);
                return;
            }

            location.href = _contextPath + "/cmm/sci/fileUpload/link.do?fileDocNo=" + encodeURIComponent(fileDocNo) + "&fileNo=" + fileNo+ "&downloadDocType=" + downloadDocType  + "&ubiYn=" + ubiYn;
        }
      ...
      , exist: function (fileDocNo, fileNo, downloadDocType = "cmm", ubiYn = "N") {
            var result = false;
            $.ajax({
                url: _contextPath + "/cmm/sci/fileUpload/selectExistFile.do?" + "fileDocNo=" + fileDocNo + "&fileNo=" + fileNo + "&downloadDocType=" + downloadDocType + "&ubiYn=" + ubiYn
                , type: 'GET'
                , async: false
                , error: function (jqXHR, status, error) {
                }
        ...
        ,downloadSec: function (fileSecKey, ubiYn) {
            if (!dms.fileManager.existSec(fileSecKey, ubiYn)) {
                dms.notification.error(dms.settings.defaultMessage.fileNotFound);
                return;
            }

            location.href = _contextPath + "/cmm/sci/fileUpload/"+fileSecKey+"/link.do?ubiYn=" + ubiYn;
        }
        ...
        , existSec: function (fileSecKey, ubiYn = "N") {
            var result = false;
            $.ajax({
                url: _contextPath + "/cmm/sci/fileUpload/selectExistFileSec.do?fileSecKey=" + fileSecKey + "&ubiYn=" + ubiYn
                , type: 'GET'
                , async: false
                , error: function (jqXHR, status, error) {
                }
