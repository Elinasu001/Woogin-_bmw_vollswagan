...
(function ($) {
    ...
    var ExtGrid = Widget.extend({
            ...
            var settings = {
                dataSource:{
                    ...
                    ,requestStart: function(e) {
                        kendo.ui.progress(gridWrapper.find("div.k-grid-content"), true);
                    }
                    ,requestEnd: function(e) {
                        kendo.ui.progress(gridWrapper.find("div.k-grid-content"), false);
                    }
                    ,transport:{
                        read:{
                            dataType:"json"
                ...
                ,multiSelectCustomWithCheckbox:false      //멀티선택 적용
                ,navigatable:true
                ,pageable:{
                    refresh:false
                    ,pageSizes:[10, 20, 30]
            ...
            if(applyFilter && applyIndvColumn){
                ...
                gridWrapper.find("a.colSetting_open").bind("click", function(){
                    ...
                    if(colSettingArea.is(":hidden")){
                        $(this).addClass("active");
                        if(colSettingArea.outerHeight() > colSettingGrid.outerHeight() && (colSettingGrid.offset().top + colSettingArea.outerHeight() + 18) > $("#cont-body").height()){
        ...
        _multiSelectWithCheckbox:function(){
            ...
            if((that.options.multiSelectWithCheckbox || that.options.multiSelectCustomWithCheckbox)
                && (that.options.selectable != "row" )){
        ...
        _checkboxToggle:function() {
            ...
            $(".checkAll", that.element).click(function() {
                ...
                if(!that.options.multiSelectCustomWithCheckbox){
	                if (checked) {
        ...
        _rowSelect:function(){
            ...
            if(that.options.multiSelectCustomWithCheckbox){
				if(that.options.selectable == 'multiple, cell'  ){
					that.tbody.find(">tr").removeClass("k-state-selected");
	                if(that.options.multiSelectLocked){
	                	that.lockedContent.find("tr").removeClass("k-state-selected");
	                }
    ...
    var ExtMaskedDatePicker = Widget.extend({
        init:function (element, options) {
			...
			function handleValidatedChange() {
			    if (options.endDatePickerId && options.startDatePickerId) {
                	dms.notification.warning(that.options.dateValidMessage);
			        console.error("endDatePickerId와 startDatePickerId 둘 중 하나만 설정할 수 있습니다.");
			        return;
			    }
			
			    var currentDate = new Date(that.value());
			
			    if (options.endDatePickerId) {
			        var endDatePicker = $("#"+options.endDatePickerId).data("kendoExtMaskedDatePicker");
			        if (currentDate) {
			            var startDate = new Date(currentDate);
			            startDate.setDate(startDate.getDate() + 1);
			            endDatePicker.min(startDate); // 종료일의 최소 선택 가능 날짜를 설정
			        }
			    }
			
			    if (options.startDatePickerId) {
			        var startDatePicker = $("#"+options.startDatePickerId).data("kendoExtMaskedDatePicker");
			        if (currentDate) {
			            var endDate = new Date(currentDate);
			            endDate.setDate(endDate.getDate() - 1);
			            startDatePicker.max(endDate); // 시작일의 최대 선택 가능 날짜를 설정
			        }
			    }
			}
			
			// 기존 change 함수에 date range validation 추가 발생
			function decoratedChangeEvent(event) {
			    if (typeof originChangeEvent === 'function') {
			        originChangeEvent.call(that, event); // 원래 함수 호출
			    }
			    handleValidatedChange(); // date range validation
			}
			
			// 이전에 정의된 CHANGE 이벤트가 있는지 확인하고 그에 따라 처리
			if (options[CHANGE]) {
			    var originChangeEvent = options[CHANGE];
			    options[CHANGE] = decoratedChangeEvent; // 기존 change 이벤트에 데코레이터 함수 적용
			} else {
			    options[CHANGE] = handleValidatedChange; // date range validation
			}
            ...
            */
            ...
            $element.data("kendoMaskedTextBox").bind("change", function(e){
                ...
                var _min = that.options.min? new Date(that.options.min.toDateString()) : null;
                var _max = that.options.max? new Date(that.options.max.toDateString()) : null;

                if (d == null || !isInRange(d, _min, _max)) {
                    $element.val("");
                    if (dms.browser.isMobile()) {
...
//            ,top:0
//            ,left:0
			,startDatePickerId : null
			,endDatePickerId : null
        }
        ,events:[
            CHANGE
            ,OPEN
