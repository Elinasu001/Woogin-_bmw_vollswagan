...
(function(window, document, $) {
	...
	sal.list = {
        ...
        */
		...
		/*
        ---------------------------------------------------------
            리스트에서 해당하는 필드의 null 데이터제외 한다.
            Ex) sal.list.removeNull(paramList, seriesCd );
         ---------------------------------------------------------
        */
		,removeNull : function(paramList, paramField ){

			return paramList.filter(
				function(data){
					return data[paramField] != null;
	...
	sal.dropDownList = {
        ...
        ---------------------------------------------------------
         ...
         ---------------------------------------------------------
        */
		seriesCd: function (asyncType, brandCd) {
			var responseJson = dms.ajax.getJson({
				url: "/sal/carMng/selectCarCommonSeriesList.do"
				, data: JSON.stringify({"sBrandCd": brandCd})
				, async: asyncType
			});
        ...
        ---------------------------------------------------------
            series1Cd 리스트를 가지고 온다.
            Ex) sal.dropDownList.series1Cd(asyncType,brandCd,useYn,saleYn);
         ---------------------------------------------------------
        */
		, series1Cd: function (asyncType, brandCd, useYn, saleYn) {
			var responseJson = dms.ajax.getJson({
        ...
        */
		...
		---------------------------------------------------------
		 ...
		 ---------------------------------------------------------
		*/
		, modelCd: function (bpsYn, asyncType, brandCd, series1Cd, useYn, series1CdList) {

			var responseJson = dms.ajax.getJson({
				url: "/sal/cmm/selectModelDropDownList.do"
				, data: JSON.stringify({"bpsYn": bpsYn, "brandCd": brandCd, "series1Cd": series1Cd, "useYn": useYn, "series1CdList": series1CdList})
				, async: asyncType
			});
        ...
        */
        ...
        ---------------------------------------------------------
         ...
         ---------------------------------------------------------
        */
		, modelYear: function (asyncType, brandCd, series1Cd, modelCd, useYn) {

			var responseJson = dms.ajax.getJson({
				url: "/sal/cmm/selectYearDropDownList.do"
				, data: JSON.stringify({"brandCd": brandCd, "series1Cd": series1Cd, "modelCd": modelCd, "useYn": useYn})
				, async: asyncType
			});
        ...
        ---------------------------------------------------------
            pkgCd 리스트를 가지고 온다.
            Ex) sal.dropDownList.pkgCd(asyncType, brandCd, series1Cd, modelCd, modelYear, useYn);
         ---------------------------------------------------------
        */
		, pkgCd: function (asyncType, brandCd, seriesCd, modelCd, modelYearCd, useYn) {

			var responseJson = dms.ajax.getJson({
				url: "/sal/cmm/selectPkgDropDownList.do"
				, data: JSON.stringify({"brandCd": brandCd, "seriesCd": seriesCd, "modelCd": modelCd, "modelYearCd": modelYearCd, "useYn": useYn})
				, async: asyncType
			});
        ...
        */
        ...
        ---------------------------------------------------------
            ...
            Ex) sal.dropDownList.description(asyncType,brandCd,series1Cd,modelCd,modelYearCd, useYn, modelCdList);
         ---------------------------------------------------------
        */
		, description: function (asyncType, brandCd, series1Cd, modelCd, modelYearCd, useYn, modelCds, series1Cds) {
        ...
        */
        ...
        ---------------------------------------------------------
         ...
         ---------------------------------------------------------
        */
		, country: function (asyncType, brandCd, series1Cd, modelCd, modelYear, pkgGrpCd, useYn) {
        ...
        */
        ...
        ---------------------------------------------------------
            대리점 리스트를 가지고 온다.
            Ex) sal.dropDownList.brchCd(asyncType,dlrCd,bizClsCd,brandList,brandCd);
         ---------------------------------------------------------
        */
		, brchCd: function (asyncType, dlrCd, bizClsCd, brandList, brandCd, bizClsCdList) {
			var responseJson = dms.ajax.getJson({
		...
		/*
        ---------------------------------------------------------
            다 건의 딜러에 관한 대리점 리스트를 가지고 온다.
            Ex) sal.dropDownList.brchCdByDlrCdList(asyncType, dlrCdList, bizClsCd, brandList);
         ---------------------------------------------------------
        */
		, brchCdByDlrCdList: function (asyncType, dlrCdList, bizClsCd, brandList) {
			var responseJson = dms.ajax.getJson({
				url: "/sal/cmm/selectSalesBrchInfoList.do",
				data: JSON.stringify({
					"sCorpCdList": dlrCdList,
					"sBizClsCd": bizClsCd,
					"sBrandList": brandList
				}),
				async: asyncType
        ...
        */
		...
		/*
        ---------------------------------------------------------
            다 건의 지점에 관한 대리점 별 팀 리스트를 가지고 온다.
            Ex) sal.dropDownList.deptCdByBrchCdList(asyncType, dlrCdList, brchCdList);
         ---------------------------------------------------------
        */
		, deptCdByBrchCdList: function (asyncType, dlrCdList, brchCdList) {

			var responseJson = dms.ajax.getJson({
				url: "/sal/cmm/selectSalesDeptInfoList.do"
				, data: JSON.stringify({
					"sCorpCdList": dlrCdList
					, "sBrchCdList": brchCdList
				})
				, async: asyncType
			});
        ...
        */
		...
		/*
        ---------------------------------------------------------
            다 건의 지점에 관한 대리점 별 SC 리스트를 가지고 온다.
            Ex) sal.dropDownList.scIdByDeptList(asyncType, dlrCdList, brchCdList, deptCdList, pstnId, pstnList);
         ---------------------------------------------------------
        */
		, scIdByDeptList: function (asyncType, dlrCdList, brchCdList, deptCdList, pstnId, pstnList) {
        ...
        ---------------------------------------------------------
         ...
         ---------------------------------------------------------
        */
		, pstnUserId: function (asyncType, dlrCd, brchCd, pstnId, brandCd) {

			var responseJson = dms.ajax.getJson({
				url: "/sal/cmm/selectSalesEmplInfoList.do"
				, data: JSON.stringify({"sCorpCd": dlrCd, "sBrchCd": brchCd, "sPstnId": pstnId, "sEmplBrandCd" : brandCd})
				, async: asyncType
			});
        ...
        */
		...
		/*
        ---------------------------------------------------------
            브랜드별 모든 스펙 리스트를 가지고 온다.
            Ex) sal.dropDownList.allSpecByBrandCd(asyncType, brandCd);
         ---------------------------------------------------------
        */
		, allSpecByBrandCd: function (asyncType, brandCd) {
			var responseJson = dms.ajax.getJson({
				url: "/sal/carMng/selectAllSpecListByBrandCd.do"
				, data: JSON.stringify({"sBrandCd": brandCd})
				, async: asyncType
			});
