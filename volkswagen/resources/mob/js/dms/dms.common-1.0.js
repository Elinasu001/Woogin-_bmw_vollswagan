...
(function(window, document, $) {
    ...
    dms.window = {
        ...
        popup:function(options){
			...
			if(!this.settings.modal){
	            if (window.parent) {
	            	 if (dms.browser.isMobile()) { // tablet 에서 팝업사이즈가 큰경우 tab-iframe 사이즈를 넘어가는 문제때문에 절대값으로 수정
	                    _popupWindowContainer.closest(".k-window").css({
	                        //top: 40, left: 20, maxWidth: 960
	                        top: 0, left: 0, width: "100%", height: "100%"
	                    });

	    				$("body").find("div.k-widget").removeClass("k-widget");
	    				$("body").find("div.k-window").removeClass("k-window");
	                }
	                else if (dms.browser.isTablet()) { // tablet 에서 팝업사이즈가 큰경우 tab-iframe 사이즈를 넘어가는 문제때문에 절대값으로 수정
	                    _popupWindowContainer.closest(".k-window").css({
	                        top: 40, left: 20, maxWidth: 960
	                    });
	                }
        ...
        popupJuso:function(options){
        	window.selectJusoPopupWin = dms.window.popup({
				windowId:"selectJusoPopupWin"
		         ...
		         ,title: false
				,content:{
					url: _contextPath + "/mob/cmm/addr/selectJusoPopup.do"
					, data:{
						"type":1
						,"callbackFunc": function(data){
    ...
    dms.fileManager = {
        ...
        ,previewSec:function(fileSecKey, popupWidth, popupHeight, popupTitle){
            if(!dms.fileManager.existSec(fileDocNo, fileNo, downloadDocType)){
                dms.notification.error(dms.settings.defaultMessage.fileNotFound);
                return;
            }
