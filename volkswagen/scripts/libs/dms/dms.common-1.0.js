...
(function(window, document, $) {
    ...
    dms.window = {
        ...
        popupPostal:function(options) {
        	...
        	if (_popupContainer.length == 0) {
	    		...
	    		$("<img/>", {src : "/resources/img/btn_close.png ", id : "btnCloseLayer", onclick : "javascript:dms.window.popupPostalOnClose('"+options.windowId+"');"})
            ...
            var element_height = 450; //우편번호서비스가 들어갈 element의 height
    ...
    dms.fileManager = {
        downloadImgDoc:function(fileDocNo, fileNo, dowloadDocType){
            location.href = _contextPath + "/cmm/sci/fileUpload/download.do?fileDocNo=" + fileDocNo + "&fileNo=" + fileNo+ "&dowloadDocType=" + dowloadDocType;
        }
        ...
        ,exist:function(fileDocNo, fileNo){
            var result = false;
            $.ajax({
                url:_contextPath + "/cmm/sci/fileUpload/selectExistFile.do?"+"fileDocNo="+fileDocNo+"&fileNo="+fileNo
                ,type:'GET'
                ,async:false
                ,error:function(jqXHR, status, error) {
                }
					...
					for(var i=0; i<len; i++){
                      ...
                      if(dataType === "input") {
						if(type === "radio") {
                         ...
                         $element.data("kendoExtNumericTextBox").value(custValue||"");
	...
	dms.corp = {
		settingCorp : function(htmlId,corpNmTypes,corpCd) {
			...
			//추후에 superadmin 계정확인시에 전체리스트 볼수있게 만들어놓은 공간
			/*if(){
