    ...
    var ExtGrid = Widget.extend({
        init:function(element, options){
            ...
            var settings = {
                dataSource:{
                    ...
                    ,schema:{
                        ...
                        ,total: function(response) {
							$(element).parent().find(".total-count-area").children().text(response.total);
                ...
                ,_columns:[]
                ,noRecords: {
                    template: "<div style='position:absolute; top:50%; left:50%; transform:translate(-50%, -50%);'>조회된 데이터가 없습니다.</div>"
                }
            };
        ...
        _multiSelectWithCheckbox:function(){
            ...
            if(that.options.multiSelectWithCheckbox && that.options.selectable != "row"){
                ...
                that.bind(CHANGE, function(e){
					//if(e.sender._current[0].cellIndex > 0){
						that._rowSelect();	
					/*} else {
						if(e.sender._current.find(".grid-checkbox-item").is(":checked")){
							e.sender._current.find(".grid-checkbox-item").prop("checked", false);	
						} else {
							e.sender._current.find(".grid-checkbox-item").prop("checked", true);
						}
						
			            if (e.sender._current.find(".grid-checkbox-item").is(":checked")) {
							//$(e.currentTarget).closest("tr").addClass("k-state-selected");
							e.sender._current.closest("tr").addClass("k-state-selected");
			            } else {
